...
label{display:block;margin-bottom:14px}input{padding:8px;font:inherit;width:85%}[contenteditable]{white-space:pre-wrap;border:1px solid #bbb;min-height:140px;padding:12px}button{margin-top:16px;padding:8px 24px}
...
try:
    ...
    browser(*launch, 'open', 'about:blank')
    browser('network', 'route', 'https://mail.google.com/**', '--body', '{}')
    browser('open', 'https://mail.google.com/mail/u/0/#inbox?holmes-synthetic-test=1')
    evaluate('document.documentElement.innerHTML = ' + json.dumps(HTML) + '; true')
    production = (ROOT / 'holmes-extension/email-compose.js').read_text()
    ...
    browser('find', 'role', 'textbox', 'click', '--name', 'Subject', '--exact')
    ...
    check(evaluate('HolmesEmailCompose.read().body === ' + json.dumps(body)), 'multiline body survives real DOM innerText readback')
    ...
    browser('find', 'role', 'textbox', 'fill', '--name', 'Subject', '--exact', 'Different topic')
